...
namespace obl
{
	...
	void taostore_circuit_2::access_thread(request_t &_req)
	{
	...
	void taostore_circuit_2::download_path(leaf_id path, std::vector<std::shared_ptr<node>> &fetched_path)
	{
	...
	std::uint64_t taostore_circuit_2::fetch_path(std::uint8_t *_fetched, block_id bid, leaf_id path)
	{
		// always start from root
		std::int64_t l_index = 0;
		int i = 0;
	...
	void taostore_circuit_2::write_thread(request_t &_req)
	{
		std::uint8_t _fetched[block_size];
		block_t *fetched = (block_t *)_fetched;
