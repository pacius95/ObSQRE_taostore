...
// 	circuit_mose_oram::circuit_mose_oram(std::size_t N, std::size_t B, unsigned int Z, unsigned int S, unsigned int T_NUM) : tree_oram(N, B, Z)
// 	{
...
// 		bucket_size = pad_bytes(sizeof(bucket_t) + sizeof(obl_aes_gcm_128bit_iv_t) * T_NUM + sizeof(obl_aes_gcm_128bit_tag_t) * T_NUM + this->Z * block_size, 8);
...
// 			chunk_idx[i] += chunk_idx[i-1] + chunk_size[i-1];
...
// 	{
// 		return ((mose_args *)object)->arg1->decription(((mose_args *)object)->arg2);
// 	}
...
// 			adata[i][idx].valid_l = tree[l_index].reach_l;
// 			adata[i][idx].valid_r = tree[l_index].reach_r;

// 			// dump left and right child mac if valid, otherwise pad with 0s
// 			if (adata[i][idx].valid_l)
// 				std::memcpy(adata[i][idx].left_mac, tree[leftch].mac[idx], sizeof(obl_aes_gcm_128bit_tag_t));

// 			if (adata[i][idx].valid_r)
// 				std::memcpy(adata[i][idx].right_mac, tree[rightch].mac[idx], sizeof(obl_aes_gcm_128bit_tag_t));
...
//         reachable = (path >> i) & 1 ? adata[i][idx].valid_r : adata[i][idx].valid_l;
...
// 					the ORAM tree
// 				*/
// 				std::uint8_t *src = ((path >> i) & 1) ? adata[i][idx].right_mac : adata[i][idx].left_mac;
...
//             reachable = (path >> i) & 1 ? adata[i][0].valid_r : adata[i][0].valid_l;
