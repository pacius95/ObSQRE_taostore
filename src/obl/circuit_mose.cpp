...
namespace obl
{
	...
	circuit_mose_oram::circuit_mose_oram(std::size_t N, std::size_t B, unsigned int Z, unsigned int S, unsigned int T_NUM) : tree_oram(N, B, Z)
	{
		...
		*/
		...
		//PASS a Value of B that is dividable by T_NUM
		block_size = pad_bytes(sizeof(block_t) + this->B, 8);
		bucket_size = pad_bytes(sizeof(bucket_t) + sizeof(obl_aes_gcm_128bit_tag_t) * T_NUM + this->Z * block_size, 8);
		...
		block_idx = new unsigned int [T_NUM];
		for (int i=0; i < T_NUM < i++)
			block_idx[i] = i*(B/T_NUM);
